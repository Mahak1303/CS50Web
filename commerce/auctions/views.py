...
from .models import User, Listing, Bid, Comment, Watchlist, Closebid, Alllisting
...
def listingpage(request,id):
    ...
    if request.user.username:
        try:
            if Watchlist.objects.get(user=user.username, listingid=id):
                added = True
        except:
            added = False
        try:
...
def bidsubmit(request, listingid):
    ...
    if request.method == "POST":
        ...
        if user_bid > current_bid:
            ...
            response = redirect('listingpage', id = listingid)
            response.set_cookie('error', 'Bid should be greater than current price')
            return response
    else:
        return redirect('index')
...
def closebid(request, listingid):
    if request.user.username:
        ...
        try: 
            ...
            pass
        try: 
            endbid = Bid.objects.filter(listingid=listingid)
            endbid.delete()
        except:
            pass
        try: 
...
def mywinnings(request):
    if request.user.username:
        items = []
        try: 
            wonitems = Closebid.objects.filter(winner=request.user.username)
            for w in wonitems:
